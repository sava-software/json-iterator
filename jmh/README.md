...
- **`IocBench`** — primitive comparisons over two real documents (a ~4.7 MiB
  ...
  search response): field-name delivery (`fieldWalk*`), dispatch at 94 field names (`dispatchTwitter*`:
...
lost to the matcher, which reuses its zero-copy plumbing internally. On the
vectorize branch all deprecated APIs have since been removed, along with the
`fieldWalkTwitter_readObjField` benchmark that measured one of them.
